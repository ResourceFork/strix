...
> ⚠️ **If flashing fails with `not in sync: resp=0x00`, you have an old-bootloader
> Nano.** Most cheap clones do. Nothing is broken — the bootloader just runs at
> 57600 baud instead of 115200, so the upload never handshakes. Pick
> **ATmega328P (Old Bootloader)** under Tools → Processor in the IDE, or with the
> CLI:
>
> ```bash
> arduino-cli upload --fqbn arduino:avr:nano:cpu=atmega328old -p /dev/cu.usbserial-XXXX arduino/ControllerTakeoverPwm
> ```
>
> This is the board used for this build, so expect to need it. Note it affects
> _uploading only_ — the sketch's own `Serial.begin(115200)` is unrelated and
> stays as it is.
...
removing from the controller _are_ resistors — wiper-to-end on a sacrificed pot
...
_ratio_, not absolute ohms. A 10 kΩ digipot on this build's ~5 kΩ pot pads
