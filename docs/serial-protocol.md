...
| `R?`     | Rail sense diagnostic (Variant B only)                                   | `R:<railInUse>:<rawA0>:<valid>`       |
...
into a stop rather than a runaway. Note that _any_ command resets the timer — the
...
`0…100` — so that `0` always lands _exactly_ on your calibrated neutral value.
